...
UDPServer::UDPServer(std::size_t port, std::string ip)
    : socket_(_io_context, asio::ip::udp::endpoint(asio::ip::make_address(ip), port)), _magic_number(std::rand())
{
...
std::string UDPServer::pack(const T &component, uint32_t entity_id)
{
    ...
    {
        Packet packet = {_magic_number, entity_id, static_cast<u_int32_t>(type_index)};
        try
        {
